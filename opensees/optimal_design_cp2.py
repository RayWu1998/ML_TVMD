...
def fitness(wave_features, original_model_resp, Rfc, model_a_max, model_a_avg, model_theta_max, model_theta_avg):
    ...
    def fitness_ga(p):
        ...
        for wave_name in original_model_resp.keys():
            ...
            a_max_predict = model_a_max.predict(input_val)[0]
            a_avg_predict = model_a_avg.predict(input_val)[0]
            drift_max_predict = model_theta_max.predict(input_val)[0]
            drift_avg_predict = model_theta_avg.predict(input_val)[0]
...
if __name__ == "__main__":
    ...
    # 导入训练好的模型
    Rfc = joblib.load("./ml_models2/rfc.model")
    model_a_avg = joblib.load("./ml_models2/rfr_a_avg.model")
    model_a_max = joblib.load("./ml_models2/rfr_a_max.model")
    model_theta_avg = joblib.load("./ml_models2/rfr_theta_avg.model")
    model_theta_max = joblib.load("./ml_models2/rfr_theta_max.model")
