...
if __name__ == "__main__":
    ...
    for k in range(save_point):
        design_res = pd.DataFrame(data=None,
                                  columns=["m1", "m2", "m3", "m4", "m5", "m6", "m7", "m8", "m9", "k1", "k2", "k3",
                                           "k4", "k5", "k6", "k7", "k8", "k9", "c1", "c2", "c3", "c4", "c5", "c6",
                                           "c7", "c8", "c9", "wave_name", "T_1", "a_max", "a_avg", "drift_max",
                                           "drift_avg"])
        for i in range(loop_num):
            for j in range(cal_num):
                design_res.loc[len(design_res)] = random_design(wave_names, NPTS_dict, dt_dict, 1)
            send_wechat(title, "保存点{}, 已完成了{}/{}_{}".format(k + 1, (i + 1) * cal_num, loop_num * cal_num,
                                                                   random.random()))
        send_wechat(title + "_saved" + str(k + 1), "保存点{}_{}".format(k + 1, random.random()))
        design_res.to_csv('./res/result_2_' + str(k + 1) + "_" + str(random.random()) + '.csv', index=False)
